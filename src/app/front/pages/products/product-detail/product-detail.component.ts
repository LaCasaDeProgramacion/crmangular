...
import { Component, OnInit, AfterViewInit, OnChanges, Input, ViewChild, ElementRef, Output, EventEmitter, Renderer2 } from '@angular/core';
...
export class ProductDetailComponent implements OnInit, OnChanges, AfterViewInit {
  public static readonly validZoomModes: string[] = ['hover', 'toggle', 'click', 'hover-freeze'];
  ...
  constructor(private route: ActivatedRoute,private productService : ApiService, private renderer: Renderer2, private elRef: ElementRef) { }
  @Input('thumbImage')
    ...
    public set setZoomMode(zoomMode: string) {
        if (ProductDetailComponent.validZoomModes.some(m => m === zoomMode)) {
